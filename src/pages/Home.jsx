    ...
    <div className="flex-grow flex py-12">
      <div className="w-1/2 flex items-center justify-center">
        <h1 className="text-4xl font-bold">
          {user ? `Bienvenue ${user.name} !` : "Bienvenue, veuillez vous connecter"}
        </h1>
      </div>
            ...
            <div className="space-y-4 w-full max-w-xs">
              ...
              {["ADMIN", "DIRECTRICE"].includes(user.role) && (
                <>
                  ...
                  <Button label="Répartition des professeurs" onClick={() => navigate("/professorDistribution")} />
                  <Button label="Clôturer l'année" color="bg-yellow-highlight" onClick={() => navigate("/closeYear")} />
                </>
              )}
              {["ADMIN", "PROFESSEUR"].includes(user.role) && (
                <>
                </>
              )}
            </div>
